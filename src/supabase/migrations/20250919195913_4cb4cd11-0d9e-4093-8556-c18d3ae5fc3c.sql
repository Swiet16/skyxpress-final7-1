-- 1. Update tracking configuration for 12-digit format
...
-- 2. Add owner role and update role system
...
-- 4. Set owner role for specific email
UPDATE public.profiles 
SET role = 'owner'
WHERE user_id = (SELECT id FROM auth.users WHERE email = 'myne7x@gmail.com');

-- 5. Update tracking ID generation function for 12-digit format
...
-- 6. Update parcels table to use new tracking ID function
ALTER TABLE public.parcels 
ALTER COLUMN tracking_id SET DEFAULT generate_12_digit_tracking();

-- 7. Add pickup status tracking to prevent multiple pickups
ALTER TABLE public.parcels 
ADD COLUMN IF NOT EXISTS is_picked_up BOOLEAN DEFAULT false;

-- 8. Create invoice metadata table for better invoice management
...
-- 9. Update RLS policies for owner role
DROP POLICY IF EXISTS "Admins and staff can manage all parcels" ON public.parcels;
CREATE POLICY "Admins, staff and owner can manage all parcels" ON public.parcels
  FOR ALL USING (
    (get_user_role(auth.uid()) = ANY(ARRAY['admin'::text, 'staff'::text, 'owner'::text])) 
    AND (NOT is_user_blocked(auth.uid()))
  );

-- Update other admin policies to include owner
DROP POLICY IF EXISTS "Admins can manage all invoices" ON public.invoices;
CREATE POLICY "Admins and owner can manage all invoices" ON public.invoices
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.profiles 
      WHERE profiles.user_id = auth.uid() 
...
-- Hide owner from regular admin queries
CREATE OR REPLACE FUNCTION public.get_visible_profiles(requesting_user_id UUID)
RETURNS TABLE (
  id UUID,
  user_id UUID, 
  full_name TEXT,
...
BEGIN
  ...
  SELECT profiles.role INTO requesting_role 
  FROM public.profiles 
  WHERE profiles.user_id = requesting_user_id;
  
  -- If owner, show all profiles
  IF requesting_role = 'owner' THEN
    RETURN QUERY
    SELECT p.id, p.user_id, p.full_name, au.email, p.role, p.is_blocked, p.created_at
    FROM public.profiles p
    LEFT JOIN auth.users au ON p.user_id = au.id;
  
  -- If admin, show all except owner
  ELSIF requesting_role = 'admin' THEN
    RETURN QUERY
    SELECT p.id, p.user_id, p.full_name, au.email, p.role, p.is_blocked, p.created_at
    FROM public.profiles p
    LEFT JOIN auth.users au ON p.user_id = au.id
  ...
  -- If regular user, show only themselves
  ELSE
    RETURN QUERY
    SELECT p.id, p.user_id, p.full_name, au.email, p.role, p.is_blocked, p.created_at
    FROM public.profiles p
